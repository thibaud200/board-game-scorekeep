...
export function GameTemplates({ gameTemplates }: GameTemplatesProps) {
  ...
  logger.debug('handleEdit called with template: ' + JSON.stringify(template));
  ...
  const handleSave = async () => {
    ...
    const template: GameTemplate = {
      id_bgg: formData.id_bgg?.trim() || undefined,
    ...
    try {
      if (editingTemplate) {
        logger.debug('UI: updateGameTemplate called for id: ' + editingTemplate.id)
        await db!.updateGameTemplate(String(editingTemplate.id), template)
  ...
  const handleDelete = async (templateId: number) => {
    ...
    if (!db) return
    try {
      await db.deleteGameTemplate(String(templateId))
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="space-y-6 overflow-y-auto flex-1 pr-2">
            ...
            <div>
              <Label htmlFor="id_bgg">BoardGameGeek ID</Label>
              <Input
                id="id_bgg"
                value={formData.id_bgg}
                onChange={(e) => setFormData(prev => ({ ...prev, id_bgg: e.target.value }))}
